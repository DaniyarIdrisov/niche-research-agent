...
**[СКРИН 3: Фрагмент `src/agents/supervisor.py` открытый в IDE — секция `build_graph()`. Подпись: «Рис. 2. Декларативное определение графа LangGraph».]**
...
**[СКРИН 6: Файл `skills/specs_normalizer/SKILL.md` открыт в IDE с подсвеченным YAML frontmatter. Подпись: «Рис. 5. Пример скилла в формате Anthropic Agent Skills».]**
...
| 6 | Любой `skills/*/SKILL.md` с YAML frontmatter | IDE | ☐ |
